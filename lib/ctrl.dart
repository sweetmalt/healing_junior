...
class MyCtrl extends GetxController {
      ...
      if (temp.length == 2) {
        ...
        if (temp[0] == "bci") {
          ...
          if (temp.length == 15) {
            ...
            pushData([att, med, delta, theta, alpha, beta, gamma]);
          }
        }
  ...
  void pushData(List<double> eegDatas) {
    double att = eegDatas[0];
    double med = eegDatas[1];
    double delta = eegDatas[2];
    double theta = eegDatas[3];
    ...
    if (wavesCtrl.dataFlSpot0.length >= 3600) {
    ...
    if ((delta > 0 && delta < upperLimit) &&
      ...
      if (bciDataDelta.length == 16) {
        ...
        //
        int baselineTotal = baselineCtrl.delta.value + baselineCtrl.theta.value + baselineCtrl.alpha.value + baselineCtrl.beta.value + baselineCtrl.gamma.value;
      ...
      if (bciDataDelta.length % 16 == 0) {
        Map<String, dynamic> deltaTrend = Data.calculateTrendSign(bciDataDelta.sublist(bciDataDelta.length - 16, bciDataDelta.length));
        Map<String, dynamic> thetaTrend = Data.calculateTrendSign(bciDataTheta.sublist(bciDataTheta.length - 16, bciDataTheta.length));
        Map<String, dynamic> alphaTrend = Data.calculateTrendSign(bciDataAlpha.sublist(bciDataAlpha.length - 16, bciDataAlpha.length));
        Map<String, dynamic> betaTrend = Data.calculateTrendSign(bciDataBeta.sublist(bciDataBeta.length - 16, bciDataBeta.length));
        Map<String, dynamic> gammaTrend = Data.calculateTrendSign(bciDataGamma.sublist(bciDataGamma.length - 16, bciDataGamma.length));
        double deltaTrendSign = deltaTrend["sign"];
        double thetaTrendSign = thetaTrend["sign"];
        double alphaTrendSign = alphaTrend["sign"];
